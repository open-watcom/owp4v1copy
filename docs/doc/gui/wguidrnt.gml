...
.keep 16
.figure *depth='2.38' *scale=125 *file='DRW1' The &drnt window contains the event list where occurring events are recorded.
.np
...
.keep 17
.figure *depth='2.57' *scale=150 *file='DRW4' The Debugging Information Processor Status dialog shows the DIP's &drnt has loaded.
.np
...
.figure *depth='1.xx' *file='DRWx' The Process Status dialog provides additional information on the program state when the exception occurred.
.np
...
.figure *depth='1.xx' *file='DRWx' The Memory Walk window displays a walk of the address space for the process that caused the exception.
.np
...
.figure *depth='1.xx' *file='DRWx' The Memory Range window displays the data, in hexadecimal form, that is stored in the selected object.
.begnote
.note Note:
...
.endstep
.keep 15
.figure *depth='2.26' *scale=200 *file='DRW9' In the Goto Offset dialog, enter the offset you want to appear at the top of the Memory Display window.
.*
.endlevel
.*
.endlevel
...
.figure *depth='1.xx' *file='DRWx' In the Annotate Log dialog box, enter text you want to include in the log file entry.
.np
...
exception.
.figure *depth='1.xx' *file='DRWx' In the Memory Dump dialog box, select the information you want to include in the log file entry.
.np
...
.figure *depth='1.xx' *file='DRWx' A warning box appears when you try to close &drnt while it is still attached to processes.
.np
...
to it.
.endstep
.figure *depth='1.xx' *file='DRWx5' The Process Control dialog displays all currently running processes.
.*
.endlevel
.*
...
.figure *depth='1.xx' *file='DRWx' The Selected Process section of the Process Control dialog displays summary information for the selected process.
.np
...
.endstep
.figure *depth='1.xx' *file='DRWx' In the Set Process Priority dialog, you choose the desired priority for the selected process.
.*
...
.endstep
.figure *depth='1.xx' *file='DRWx' From the Thread Control dialog box, you can terminate, suspend, and resume threads.
.np
...
.endbull
.figure *depth='1.xx' *file='DRWx' The Memory Information dialog summarizes the memory usage of the selected process.
.np
