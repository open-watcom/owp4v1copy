...
.errnote W124 Comparison result always %d
.np
The line contains a comparison that is alway true (1) or false (0).
For example comparing an unsigned expression to see if it is >=, <, or <= to 0
is redundant.
Check to see if the expression should be signed instead of unsigned.

.errnote W125 Nested include depth of %d exceeded
.np
...
.errnote W303 Parameter '%s' has been defined, but not referenced
.np
...
.errnote E1174 Invalid constant
.np
The token sequence does not represent a valid numeric constant.

.errnote E1175 Too many initializers
.np
...
.errnote E1176 Parameter %d, pointer type mismatch
.np
You have two pointers that either point to different objects, or the
pointers are of different size, or they have different modifiers.

.errnote E1177 Modifier repeated in declaration
.np
You have repeated the use of a modifier like "const" (an error)
or "far" (a warning) in a declaration.

.errnote E1178 Type qualifier mismatch
.np
You have two pointers that have different "const" or "volatile" qualifiers.

.errnote E1179 Parameter %d, type qualifier mismatch
.np
You have two pointers that have different const or "volatile" qualifiers.

.errnote E1180 Sign specifier mismatch
.np
You have two pointers that point to types that have different sign specifiers.

.errnote E1181 Parameter %d, sign specifier mismatch
.np
You have two pointers that point to types that have different sign specifiers.
