...
set books5=clib cpplib wccerrs wpperrs wrcerrs cmix isv isvcpp isvfor
...
set books5=clib cpplib wccerrs wpperrs cmix isv isvcpp isvfor
set books6=f_readme fuguide fpguide ftools f77graph f77lr wfcerrs fmix
goto GOTBOOKS
:IBBOOKS
...
for %%i in (%books1%) do call $$$tmp.bat %%i %%i.%b1%
for %%i in (%books2%) do call $$$tmp.bat %%i %%i.%b1%
for %%i in (%books3%) do call $$$tmp.bat %%i %%i.%b1%
for %%i in (%books4%) do call $$$tmp.bat %%i %%i.%b1%
for %%i in (%books5%) do call $$$tmp.bat %%i %%i.%b1%
for %%i in (%books6%) do call $$$tmp.bat %%i %%i.%b1%
