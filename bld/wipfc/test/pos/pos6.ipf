:userdoc.
:title.Positive 6
:docprof.
:h1 res=1.List Tests
:i1 id='1'.Lists
:p.
The following pages test various list tags&per.

:h2 res=2.Simple List
:i2 refid='1'.sl
:p.The following is an example of a simple list&per.

:sl.
:li.First item&per. First item&per. First item&per. First item&per. First item&per.
:sl compact.
:li.First subitem of first item&per. First subitem of first item&per. First subitem of first item&per.
:li.Second subitem of first item&per. Second subitem of first item&per. Second subitem of first item&per.
:li.Third subitem of first item&per. Third subitem of first item&per. Third subitem of first item&per.
:li.Fourth subitem of first item&per.Fourth subitem of first item&per. Fourth subitem of first item&per.
:esl.
:li.Second item&per. Second item&per. Second item&per. Second item&per. Second item&per.
:li.Third item&per. Third item&per. Third item&per. Third item&per. Third item&per.
:sl compact.
:li.First subitem of third item&per. First subitem of third item&per. First subitem of third item&per.
:li.Second subitem of third item&per. Second subitem of third item&per. Second subitem of third item&per.
:li.Third subitem of third item&per. Third subitem of third item&per. Third subitem of third item&per.
:esl.
:esl.
:p.Text after the list&per.

:h2 res=3.Unordered List
:i2 refid='1'.ul
:p.The following is an example of an unordered list&per.
:ul.
:li.First item&per. First item&per. First item&per. First item&per. First item&per.
:ul compact.
:li.First subitem of first item&per. First subitem of first item&per. First subitem of first item&per.
:li.Second subitem of first item&per. Second subitem of first item&per. Second subitem of first item&per.
:ul compact.
:li.First subsubitem of the first list&per. First subsubitem of the first list&per. First subsubitem of the first list&per.
:li.Second subsubitem of the first list&per.Second subsubitem of the first list&per. Second subsubitem of the first list&per.
:eul.
:li.Third subitem of first item&per. Third subitem of first item&per. Third subitem of first item&per.
:li.Fourth subitem of first item&per.Fourth subitem of first item&per. Fourth subitem of first item&per.
:eul.
:li.Second item&per. Second item&per. Second item&per. Second item&per. Second item&per.
:li.Third item&per. Third item&per. Third item&per. Third item&per. Third item&per.
:ul compact.
:li.First subitem of third item&per.First subitem of third item&per. First subitem of third item&per.
:li.Second subitem of third item&per. Second subitem of third item&per. Second subitem of third item&per.
:li.Third subitem of third item&per. Third subitem of third item&per. Third subitem of third item&per.
:eul.
:eul.
:p.Text after the list

:h2 res=4.Ordered List
:i2 refid='1'.ol
:p.The following is an example of an ordered list&per.
:ol.
:li.First item&per. First item&per. First item&per. First item&per. First item&per.
:ol compact.
:li.First subitem of first item&per. First subitem of first item&per. First subitem of first item&per.
:li.Second subitem of first item&per. Second subitem of first item&per. Second subitem of first item&per.
:ol compact.
:li.First subsubitem of the first list&per. First subsubitem of the first list&per. First subsubitem of the first list&per.
:li.Second subsubitem of the first list&per.Second subsubitem of the first list&per. Second subsubitem of the first list&per.
:eol.
:li.Third subitem of first item&per. Third subitem of first item&per. Third subitem of first item&per.
:li.Fourth subitem of first item&per.Fourth subitem of first item&per. Fourth subitem of first item&per.
:eol.
:li.Second item&per. Second item&per. Second item&per. Second item&per. Second item&per.
:li.Third item&per. Third item&per. Third item&per. Third item&per. Third item&per.
:ol compact.
:li.First subitem of third item&per.First subitem of third item&per. First subitem of third item&per.
:li.Second subitem of third item&per. Second subitem of third item&per. Second subitem of third item&per.
:li.Third subitem of third item&per. Third subitem of third item&per. Third subitem of third item&per.
:eol.
:eol.
:p.Text after the list

:h2 res=5.Mixing lists together
:i2 refid='1'.ol and ul together
:p.Ordered list containing an unordered list&per.
:ol.
:li.First item&per. First item&per. First item&per. First item&per. First item&per.
:ul compact.
:li.First subitem of first item&per. First subitem of first item&per. First subitem of first item&per.
:li.Second subitem of first item&per. Second subitem of first item&per. Second subitem of first item&per.
:ol compact.
:li.First subsubitem of the first list&per. First subsubitem of the first list&per. First subsubitem of the first list&per.
:li.Second subsubitem of the first list&per.Second subsubitem of the first list&per. Second subsubitem of the first list&per.
:eol.
:li.Third subitem of first item&per. Third subitem of first item&per. Third subitem of first item&per.
:li.Fourth subitem of first item&per.Fourth subitem of first item&per. Fourth subitem of first item&per.
:eul.
:li.Second item&per. Second item&per. Second item&per. Second item&per. Second item&per.
:li.Third item&per. Third item&per. Third item&per. Third item&per. Third item&per.
:ul compact.
:li.First subitem of third item&per.First subitem of third item&per. First subitem of third item&per.
:li.Second subitem of third item&per. Second subitem of third item&per. Second subitem of third item&per.
:li.Third subitem of third item&per. Third subitem of third item&per. Third subitem of third item&per.
:eul.
:eol.
:p.Text after the list

:h2 res=6.Definition List
:i2 refid='1'.dl
:p.The following is an example of a definition list&per.
:dl.
:dt.:hp2.First term&per.:ehp2.
:dd.Definition of the first term&per.
:dt.:hp2.Second term&per.:ehp2.
:dd.Definition of the second term&per.
:dt.:hp2.Third term&per.:ehp2.
:dd.Definition of the third term&per.
:edl.
:p.With breaks between term and definition&per.
:dl break=all.
:dt.:hp2.First term&per.:ehp2.
:dd.Definition of the first term&per.
:dt.:hp2.Second term&per.:ehp2.
:dd.Definition of the second term&per.
:dt.:hp2.Third term&per.:ehp2.
:dd.Definition of the third term&per.
:edl.
:p.A compact definition list&per.
:dl tsize=15 compact.
:dthd.:hp2.Term:ehp2.
:ddhd.:hp2.Definition:ehp2.
:dt.First term&per.
:dd.Definition of the first term&per.
:dt.Second term&per.
:dd.Definition of the second term&per.
:dt.Third term&per.
:dd.Definition of the third term&per.
:edl.
:p.Text after the list&per.

:h2 res=7.Parameter List
:i2 refid='1'.parml
:p.The following is an example of a parameter list&per.
:parml.
:pt.:hp2.First term&per.:ehp2.
:pd.Definition of the first term&per.
:pt.:hp2.Second term&per.:ehp2.
:pd.Definition of the second term&per.
:pt.:hp2.Third term&per.:ehp2.
:pd.Definition of the third term&per.
:eparml.
:p.With breaks between term and definition&per.
:parml break=all.
:pt.:hp2.First term&per.:ehp2.
:pd.Definition of the first term&per.
:pt.:hp2.Second term&per.:ehp2.
:pd.Definition of the second term&per.
:pt.:hp2.Third term&per.:ehp2.
:pd.Definition of the third term&per.
:eparml.
:p.A compact definition list
:parml tsize=15 compact.
:pt.First term&per.
:pd.Definition of the first term&per.
:pt.Second term&per.
:pd.Definition of the second term&per.
:pt.Third term&per.
:pd.Definition of the third term&per.
:eparml.
:p.Text after the list&per.

:euserdoc.

