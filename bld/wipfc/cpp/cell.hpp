...
public:
    ...
    void addByte( std::uint8_t c ) { text.push_back( c ); };
    //add an escape sequence to the encoded text
    void addEsc( const std::vector< std::uint8_t >& esc );
...
private:
    ...
    std::vector< std::uint16_t > localDictionary;  //indexes into global dictionary
    typedef std::vector< std::uint16_t >::iterator LDIter;
    typedef std::vector< std::uint16_t >::const_iterator ConstLDIter;
    std::vector< std::uint8_t > text;       //indexes into local dictionary
    typedef std::vector< std::uint8_t >::iterator TextIter;
    typedef std::vector< std::uint8_t >::const_iterator ConstTextIter;
...
#pragma pack(push, 1)
struct EscSeq {
    std::uint8_t esc;                  //=0xFF
    std::uint8_t size;                 //2 + optional arguments
    std::uint8_t type;
...
0x12:   set left margin fit
        ...
        Arguments: 
            std::uint8_t margin; in spaces, 0 == no margin

0x03:   set right margin
        Generated by: rm
        Arguments: 
            std::uint8_t margin; in spaces, 1 == no margin
...
0x07:   link to footnote (:fn. tag)
        Generated by: link
        Arguments: 
            std::uint16_t tocIndex;  // toc entry number of text
