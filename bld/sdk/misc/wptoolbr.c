...
toolbar *ToolBarInit( HWND parent )
{
    ...
    if( hInstCommCtrl == NULL ) {
        ...
        if( !gdiObjectsCreated ) {
            ...
            btnShadowPen = _wpi_createpen( PS_SOLID, BORDER_WIDTH( bar ),
                                           clr_btnshadow );
            btnHighlightPen = _wpi_createpen( PS_SOLID, BORDER_WIDTH( bar ),
                                              clr_btnhighlight );
    ...
    if( hInstCommCtrl != NULL ) {
        ...
        if( LOBYTE(LOWORD(GetVersion())) >= 4 && ( !bar->is_fixed ) ) {
            CreateWindowEx( WS_EX_TOOLWINDOW, className, NULL, disp->style,
                            disp->area.left, disp->area.top, width, height,
                            bar->owner, (HMENU) HNULL, GET_HINSTANCE( bar->owner ),
                            bar );
        } else {
 ...
 */
static void drawTopLeftInsideCorner( WPI_PRES pres, WPI_POINT size, int border,
                                     HPEN pen )
{
    HPEN        old_pen;
    WPI_POINT   pt;
                        ...
                        WPI_PRES mempres, HDC mem )
{
    ...
    WPTB_TransparentBlt( mem2, dst_org.x, dst_org.y, dst_size.x, dst_size.y, mempres,
                         cr );
    if( oldbmp != HNULL ) {
    ...
    case WM_MOUSEMOVE:
        ...
        if( currTool ) {
            if( tool == currTool ) {
                if( !currIsDown ) {
                    currIsDown = TRUE;
                    drawButton( hwnd, currTool, TRUE, NULLHANDLE, NULLHANDLE,
                                NULLHANDLE );
                    if( bar->helphook != NULL ) {
                        bar->helphook( hwnd, MPFROMSHORT( currTool->id ), TRUE );
                    }
                ...
                if( currIsDown ) {
                    drawButton( hwnd, currTool, FALSE, NULLHANDLE, NULLHANDLE,
                                NULLHANDLE );
                    currIsDown = FALSE;
                    if( bar->helphook != NULL ) {
                        bar->helphook( hwnd, MPFROMSHORT( currTool->id ), FALSE );
...
void WPTB_TransparentBlt( HDC hDC, UINT x, UINT y, UINT width, UINT height, HDC hDCIn,
                          COLORREF cr )
{
   HDC      hDCMid, hMemDC;
