...
static void WdeDebugInterpretParam ( WdeDebugParamTypes type, void *param,
                                     char *temp )
{
    ...
    switch ( type ) {
        ...
        case Action:
            ...
            if ( act_item->act == -1 ) {
                sprintf(temp, "%7s: %21.21d, ", WdeDebugParamNames[type],
                                           *((ACTION *)param) );
            } else {
                sprintf(temp, "%7s: %s, ", WdeDebugParamNames[type],
                                           act_item->act_name );
            }
            break;
        ...
        case Req:
            if ( ((SUBOBJ_REQUEST *)param)->a.ty == ALL ) {
                ...
                sprintf(temp, "%7s: BY_POINT (%d,%d), ",
                              WdeDebugParamNames[type],
                              ((SUBOBJ_REQUEST *)param)->p.pt.x,
                              ((SUBOBJ_REQUEST *)param)->p.pt.y );
            } else {
                sprintf(temp, "%7s: AT_POINT (%d,%d), ",
                              WdeDebugParamNames[type],
                              ((SUBOBJ_REQUEST *)param)->p.pt.x,
                              ((SUBOBJ_REQUEST *)param)->p.pt.y );
            }
