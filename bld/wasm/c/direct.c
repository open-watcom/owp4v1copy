...
#define MAGIC_FLAT_GROUP        GetGrpIdx( &ModuleInfo.flat_grp->sym )
...
/*****************/
{
    ...
    case T_SEGMENT:
        ...
        if( !ModuleInfo.mseg
            && ( CurrSeg->seg->e.seginfo->use_32 != ModuleInfo.use32 ) ) {
            ModuleInfo.mseg = TRUE;
        }
        if( CurrSeg->seg->e.seginfo->use_32 ) {
...
{
    /**/myassert( sym != NULL );
    return( ((dir_node *)sym)->e.extinfo->idx );
}
