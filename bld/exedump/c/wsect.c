...
/********************************************************/
{
    readable_name *a = (readable_name *)pa;
    readable_name *b = (readable_name *)pb;
    if( a->value > b->value ) {
        return( 1 );
...
decode_form:
        ...
        case DW_FORM_data2:
            ...
            break;
        case DW_FORM_ref2:
            Puthex( info->cu_header + *(uint_16 *)p, 4 );
            Wdputslc( "\n" );
            p += sizeof( uint_16 );
            break;
        case DW_FORM_data4:
            if( is_loc ){
                DmpLocList( *(uint_32 *)p, info->addr_size );
            }else{
                Puthex( *(uint_32 *)p, 8 );
                Wdputslc( "\n" );
            }
            ...
            break;
        case DW_FORM_ref4:
            Puthex( info->cu_header + *(uint_32 *)p, 8 );
            Wdputslc( "\n" );
            p += sizeof( uint_32 );
            break;
        case DW_FORM_flag:
        ...
        while( p - unit_base < unit_length ) {
            ...
            if( op_code < REF_CODE_BASE ) {
                ...
                case REF_BEGIN_SCOPE:
                    Puthex( *(uint_32 *)p, 8 );
                    ScopePush( &registers.scope, *(uint_32 *)p );
                    p += sizeof( uint_32 );
                    break;
                case REF_END_SCOPE:
...
/*****************************************************/
{
    ...
    while( p  < end ) {
        ...
        Puthex( unit_length, 8 );
        Wdputslc( "\nVersion: " );
        Puthex( *(uint_16*)(p+4), 2 );
        Wdputslc( "\nDbg Info: " );
        dbg_offset =  *(uint_32 *)(p+6);
        Puthex( dbg_offset, 8 );
        Wdputslc( "\nDbg Length: " );
        dbg_length =  *(uint_32 *)(p+10);
        Puthex( dbg_length, 8 );
        Wdputslc( "\n" );
        p += 14;
