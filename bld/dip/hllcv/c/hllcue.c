                    ...
                    void *d )
{
    ...
    hdd = FindDirEntry( ii, im, hll_sstHLLSrc );
    if( hdd == NULL ) return( WR_CONTINUE );
    // TODO: HL3 style linnum records
    /* Need to read the entire subsection - don't know how big
     * the file table entries might be.
     */
    hdr4 = VMBlock( ii, hdd->lfo, hdd->cb );
    if( hdr4 == NULL ) return( WR_FAIL );
    file_tab_count = hdr4->numEntries;
    ic->im = im;
    ic->pair = 0;
    for( i = 0; i < file_tab_count; ++i ) {
        ic->file = hdd->lfo;// + file_off[i];
        fp4 = VMBlock( ii, ic->file, sizeof( *fp4 ) );
        if( fp4 == NULL ) return( WR_FAIL );
        ic->line = hdd->lfo;// + fp4->baseSrcLn[0];
        wr = wk( ii, ic, d );
                ...
                imp_cue_handle *ic, int adj, imp_cue_handle *aic )
{
    ...
    hdd = FindDirEntry( ii, ic->im, sstSrcModule );
    if( hdd == NULL ) {
    ...
    *aic = *ic;
    ok = DS_OK;
    while( adj > 0 ) {
        status = AdjForward( ii, hdd->lfo, aic );
        if( status & DS_ERR ) return( status );
        if( status != DS_OK ) ok = status;
        --adj;
    }
    while( adj < 0 ) {
        status = AdjBackward( ii, hdd->lfo, aic );
        if( status & DS_ERR ) return( status );
        if( status != DS_OK ) ok = status;
        ++adj;
    }
                ...
                unsigned column, imp_cue_handle *ic )
{
    ...
    column = column;
    hdd = FindDirEntry( ii, im, sstSrcModule );
    if( hdd == NULL ) return( SR_NONE );
    if( file == 0 ) {
        hdr = VMBlock( ii, hdd->lfo, sizeof( *hdr ) );
        if( hdr == NULL ) return( SR_NONE );
        file = hdr->baseSrcFile[0] + hdd->lfo;
    }
    ic->im = im;
    ic->file = file;
    ...
    for( seg_idx = 0; seg_idx < num_segs; ++seg_idx ) {
        line_base = line_off[seg_idx] + hdd->lfo;
...
unsigned SearchOffsets( imp_image_handle *ii, virt_mem base,
                        unsigned num_off, addr_off  want_off,
                        addr_off *best_off, addr_off  adj_off )
{
    int         lo_idx;
    int         hi_idx;
    int         idx;
                ...
                imp_mod_handle im, address addr, imp_cue_handle *ic )
{
    ...
    hdr = VMBlock( ii, hdd->lfo, sizeof( *hdr ) + file_tab_size );
    ...
    for( file_idx = 0; file_idx < num_files; ++file_idx ) {
        file_base = files[file_idx] + hdd->lfo;
        ...
        for( seg_idx = 0; seg_idx < num_segs; ++seg_idx ) {
            line_base = lines[seg_idx] + hdd->lfo;
            lp = VMBlock( ii, line_base, sizeof( *lp ) );
            if( lp == NULL ) return( SR_NONE );
