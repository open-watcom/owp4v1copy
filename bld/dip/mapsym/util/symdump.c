...
static bool isSymFile( FILE *f )
{
    ...
    /* Note: Could check that (file size - sizeof( sym_endmap )) is
     * evenly divisible by 16; this should always be the case due to
     * 16-bit alignment of maps.
     */
    ...
    if( (end_map.major_ver != SYM_VERSION_MAJOR) || (end_map.minor_ver != SYM_VERSION_MINOR)) {
        return( FALSE );
    }
...
// Dump a number of 16-bit symbols
static void dumpSyms( FILE *f, int count )
{
    sym_symdef      sym;
    char            name[256];
    int             i;
...
// Dump a number of 32-bit symbols
static void dumpSyms32( FILE *f, int count )
{
    sym_symdef_32   sym;
    char            name[256];
    int             i;
    ...
    for( i = 0; i < count; ++i ) {
        fread( &sym, 1, SYM_SYMDEF_32_FIXSIZE, f );
        readString( f, name );
        printf( "    0x%08x : '%s'\n", sym.offset, name );
    }
}

// Dump chain of segments
static void dumpSegments( FILE *f )
{
    sym_segdef      seg;
    char            name[256];
    ...
    do {
        ...
        printf( "  load address 0x%04x, phys addr 0x%04x, flags 0x%02x\n",
        ...
        if( seg.sym_type & SYM_FLAG_32BIT )
            dumpSyms32( f, seg.num_syms );
        else
            dumpSyms( f, seg.num_syms );

        fseek( f, SYM_PTR_TO_OFS( seg.next_ptr ), SEEK_SET );
    } while( seg.next_ptr != 0 );
}
...
/********************************/
{
    ...
    while( map.next_ptr != 0 ) {
        ...
        printf( "%d abs symbol(s), abs symbol table at 0x%06x, %d segments\n",
